...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 3.3b.1 | Inspect mask propagation through full pipeline (tensor_adapter→router→MTO→CG→gate→readout→checkpoint) and fix all gaps (router mask support, CG mask support, train_mu.py k_policy wiring) | All modules accept and correctly use mode_mask; k_policy=valence_half flows end-to-end; all 210 tests pass | 3.3a | cc:完了 [645641a] |
| 3.3b.2 | Dataset valence audit for QM9S subset_medium: compute N_val, K_half distributions, produce tables/figures/report, recommend K_max | `outputs/tables/qm9s_valence_k_distribution.csv` + `outputs/reports/qm9s_valence_k_audit.md`; K_max=32 recommended (K_half_max=26, p99=23) | 3.3b.1 | cc:完了 [645641a] |
| 3.3b.3 | Implement config options: k_policy, k_max, k_min, k_rounding, k_cap_policy in MTOConfig + new config file mto_valence_half.yaml | `configs/model/mto_valence_half.yaml` works with --dry-run; fixed-K configs unchanged; `valence_adaptive`→`valence_half` normalized | 3.3b.2 | cc:完了 [645641a] |
| 3.3b.4 | Add/update tests: valence_half K computation, mode_mask shapes, inactive-mode isolation, routing-softmax-over-active, vector-readout-ignores-inactive, top-r-masking-never-inactive, fwd/bwd/checkpoint, batch isolation | `pytest tests/test_valence_adaptive_k.py -v` 19 tests all pass | 3.3b.3 | cc:完了 [645641a] |
| 3.3b.5 | Smoke training: valence_half vs fixed-K=8 on 5000 molecules, 10 epochs, seed=0, target=mu | FK8 10 epochs complete (vec_mae=0.075, r²=0.996); VH 7 epochs (vec_mae=0.087, r²=0.994). Both stable. | 3.3b.4 | cc:完了 [93836/93835] |
| 3.3b.6 | Full diagnostics: K_eff absolute/relative, entropy, PR, top-r masking, LOMO, order masking, correlations, gate stats | K_entropy/K_bank=0.45 (sparse!); Gini=0.59; dead_frac=57%; absolute K_entropy=8.6 vs 19.3 capacity. All CSVs written. | 3.3b.5 | cc:完了 [93864] |
| 3.3b.7 | Write outputs: tables, figures, report answering all 10 questions | `outputs/reports/phase3_3_valence_half_mu_analysis.md` — all 10 Q&A answered, recommend full QM9S launch with valence_half K_max=32 | 3.3b.6 | cc:完了 [645641a] |
